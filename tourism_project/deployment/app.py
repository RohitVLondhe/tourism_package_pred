...
# Download Xtrain.csv to get the correct column order
try:
    xtrain_path_hf = hf_hub_download(repo_id="Pvt-Pixel/tourism-package-pred", filename="Xtrain.csv", repo_type="dataset")
    xtrain_df = pd.read_csv(xtrain_path_hf)
    expected_columns = xtrain_df.columns.tolist()
    st.success("Expected columns loaded from Xtrain.csv.")
except Exception as e:
    st.error(f"Error loading expected columns from Xtrain.csv: {e}")
    st.stop()
...
numberofpersonvisiting = st.number_input("NumberOfPersonVisiting", min_value=1, max_value=10, value=2, help="Total number of people accompanying the customer on the trip.")
preferredpropertystar = st.number_input("PreferredPropertyStar", min_value=1, max_value=5, value=3, help="Preferred hotel rating by the customer (1-5 stars).")
maritalstatus = st.selectbox("Marital Status", list(MARITAL_STATUS_MAP.keys()), help="Marital status of the customer.")
numberoftrips = st.number_input("NumberOfTrips", min_value=0, max_value=50, value=5, help="Average number of trips the customer takes annually.")
passport = st.selectbox("Passport", [0, 1], format_func=lambda x: "Yes" if x == 1 else "No", help="Whether the customer holds a valid passport (0: No, 1: Yes).")
owncar = st.selectbox("OwnCar", [0, 1], format_func=lambda x: "Yes" if x == 1 else "No", help="Whether the customer owns a car (0: No, 1: Yes).")
numberofchildrenvisiting = st.number_input("NumberOfChildrenVisiting", min_value=0, max_value=5, value=0, help="Number of children below age 5 accompanying the customer.")
...
monthlyincome = st.number_input("MonthlyIncome", min_value=0, max_value=200000, value=50000, help="Gross monthly income of the customer.")

# User input for Customer Interaction Data
st.header("Customer Interaction Data")
pitchsatisfactionscore = st.number_input("PitchSatisfactionScore (1-5)", min_value=1, max_value=5, value=3, help="Score indicating the customer's satisfaction with the sales pitch.")
productpitched = st.selectbox("ProductPitched", list(PRODUCT_PITCHED_MAP.keys()), help="The type of product pitched to the customer.")
numberoffollowups = st.number_input("NumberOfFollowups", min_value=0, max_value=10, value=2, help="Total number of follow-ups by the salesperson after the sales pitch.")
durationofpitch = st.number_input("DurationOfPitch (minutes)", min_value=0, max_value=120, value=15, help="Duration of the sales pitch delivered to the customer.")

# Assemble input into a dictionary
input_data_dict = {
    'Age': age,
...
input_data = pd.DataFrame([input_data_dict], columns=expected_columns)
